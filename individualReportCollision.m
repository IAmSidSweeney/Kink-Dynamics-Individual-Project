...
z1 = -gamma*(x - x0); % kink at x0=-10 moving to the right
z2 = gamma*(x + x0); % kink at x0=+10 moving to the left

phi0 = 4*(atan(exp(z1)) + atan(exp(z2))) - 2*pi;
v0   = -4*speed*gamma*(sech(z1) + sech(z2) );
...
tEnd = 100; dt = 0.001; time = 0:dt:tEnd;
tspan = [0,tEnd];
...
[T, Y] = ode15s(@(t,y) kink_rhs(y,Laplacian,U_p), tspan, y0);
state = Y(:, 1:N)';
...
%% ======================================= Animations =======================================
% Save animation as MP4 (using ode15s solver)
filename = append('kink_collision_animation.mp4');
